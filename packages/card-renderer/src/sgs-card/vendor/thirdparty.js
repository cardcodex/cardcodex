...
export function prepare() {
  ...
  if (typeof CUSTOM_FONTS != "undefined") {
    ...
    document.getElementById("toolbarFont").innerHTML += fontOptions.join("");
...
export function addSkill() {
  ...
  node.innerHTML =
    '<label>　　技能：</label><textarea id="panel-skill-' +
    length +
    '" title="输入技能名与说明。第一行为技能名，第二行及以后是技能说明。" onfocus="setTextToolbar(this)" on-blur="setTextToolbar()" onkeydown="editHotkey(event)"></textarea> <div class="panel-skill-tags" title="可以设置主公技、锁定技、限定技等特定样式。"><label><input type="checkbox">主</label><label> <input type="checkbox">锁</label><label><input type="checkbox">限</label> <label><input type="checkbox">衍</label></div>';
...
export function revokeURL(file) {
  var getUrl = null;
...
export function getImagePathFront() {
...
export function exportJson() {
  ...
  document.getElementById("panel-json-code").value = JSON.stringify(generateFromInput(), null, 4);
}
...
export function generateFromInput() {
  ...
  var pkg = document.getElementById("panel-package").value;
  ...
  if (adjust.length == 3 || illuFront) {
    ...
    illustration = {
      ...
      pathFront: illuFront,
      adjust: {
        x: parseFloat(adjust[0]),
        y: parseFloat(adjust[1]),
        scale: parseFloat(adjust[2])
      }
    };
  }
  ...
  var onload = function () {
    ...
    if (extra) {
      ...
      for (var i = 0; i < extra.length; i += 2) {
        ...
        if (type == "enum") {
          var values = style
            .getPropertyValue("--extra-" + key + "-values")
            .trim()
            .split(/\s*,\s*/);
          ...
          extraHTML +=
            "<span><label for='panel-x-" +
            key +
            "'>" +
            extra[i] +
            "：</label><select id='panel-x-" +
            key +
            "' class='short' " +
            value +
            ">'" +
...
`;
  ...
  if (object.kingdom) {
    ...
    if (kingdom.length >= 2) {
      var c1 = kingdom.charCodeAt(0),
        c2 = kingdom.charCodeAt(1);
  ...
  nameElement.setAttribute("value", name.replace(/<[^>]+>/g, ""));
  nameElement.setAttribute("length", nameLength);
  if (nameLength > 4) {
  ...
  hitpoints.setAttribute("maxhp", drained ? hp + drained : hp - overflow);
  hitpoints.setAttribute("total", drained ? hp + drained + overflow : hp);
  for (var i = 1, limit = Math.min(drained ? hp : hp - overflow, 100); i <= limit; ++i) {
  ...
  if (overflow > 0) {
    hitpoints.setAttribute("overflow", overflow);
  ...
  if (pkg) {
    ...
    if (pkg.length > 8 && pkg.indexOf(":") >= 0) {
      ...
      packageElement.innerHTML = '<img src="' + pkg + '">';
      packageElement.setAttribute("image", "");
    } else {
      packageElement.innerHTML = pkg;
      packageElement.setAttribute("length", getTextLength(pkg));
      packageElement.setAttribute("value", pkg);
    }
  }
  ...
  if (card) {
    ...
    if (card.clientHeight > maxheight || card.clientWidth > maxwidth) {
      ...
      result.style.transform =
        "translate(" +
        (card.clientWidth * (rate - 1)) / 2 +
        "px, " +
        (card.clientHeight * (rate - 1)) / 2 +
        "px) scale(" +
        rate +
        ")";
    } else {
...
export function exportForm() {
  ...
  for (var i = 0; i < skills.length; ++i) {
    ...
    var tagElements = skillsElement.children[i]
      .getElementsByClassName("panel-skill-tags")[0]
...
export function convertText(element, pattern, replace) {
...
export function popupHintBalloon(event) {
...
export function exportCardHTML() {
  ...
  return html
    ...
    .replace(/<body>[\s\S]+?<\/body>/, "<body>" + cardHtml + "</body>");
}

export function openCardHTML() {
...
export function startCrop() {
  ...
  if (file) {
    ...
    image.onload = function () {
      ...
      function equalPixel(offset) {
        return (
          Math.abs(d[offset] - xr) <= 32 && Math.abs(d[offset + 1] - xg) <= 32 && Math.abs(d[offset + 2] - xb) <= 32
        );
      }
      function unequalPixel(offset) {
      ...
      var left = Math.floor(width0 / 2),
        right = Math.ceil(width0 / 2);
      for (var i = top; i < bottom; ++i) {
...
export function showTrimPanel(show) {
  document.getElementById("panel-trim").style.display = show ? "" : "none";
}

export function loadMobileSrcs(style) {
  // 用于移动版的解包
  var list = [];
  var i = 0,
    prop;
...
export function setContainImageSize() {
...
export function loadFontPackage() {
  ...
  if (cssElement) {
    ...
    face.setAttribute(
      "style",
      "position: fixed; z-index: 1000; background-color: white; opacity: 0.5; left: 0; right: 0; top: 0; bottom: 0;"
    );
          ...
          fr.onloadend = function () {
            ...
            while (offset < length) {
              ...
              var n = Array.prototype.map
                .call(new Uint8Array(content.slice(offset, offset + l)), function (e) {
...
var toolbarFont = false;
export function setTextToolbar(element) {
...
export function toggleTagPairs(prefix, postfix) {
  if (currentTextArea) {
