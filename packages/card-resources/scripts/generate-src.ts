...
styleDirs.forEach(dir => {
  ...
  const styleEntryFileContent = `// Auto-generated for [${dir}] by "script/generate-src.ts"\n// please run "npm run generate:src" to update this file.\n\n${cssImport}\n${imageImports}\n${imageExports}`;
...
const CardNameMap = `export const CardResourcesKeys = [${styleDirs.map(dir => `'${dir.toUpperCase().replace(/-/g, "_")}'`).join(", ")}] as const;

export const CardResourcesValues = [${styleDirs.map(dir => `"${dir}"`).join(", ")}] as const;
...
const typesExport = `export type CardResourcesTypes = typeof CardResources[keyof typeof CardResources];
`;
const mainIndexContent = `// This file is auto-generated by "script/generate-src.ts"
// please run "npm run generate:src" to update this file.
${jsImport}
