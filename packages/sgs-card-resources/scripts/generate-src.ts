...
const CardNameMap = `export const SgsCardIdentifiers = [${styleDirs.map(dir => `'${dir.toUpperCase().replace(/-/g, "_")}'`).join(", ")}] as const;

export const SgsCardKeys = [${styleDirs.map(dir => `"${dir}"`).join(", ")}] as const;

export type SgsCardIdentifier = typeof SgsCardIdentifiers[number];

export type SgsCardKey = typeof SgsCardKeys[number];

export const SgsCardResourcesMap: Record<SgsCardIdentifier, SgsCardKey>= {
${styleDirs.map(dir => `  '${dir.toUpperCase().replace(/-/g, "_")}': '${dir}',`).join("\n")}
} as const;`;

const typesExport = `export type SgsCardResourceMap = typeof SgsCardResources[keyof typeof SgsCardResources];
`;
